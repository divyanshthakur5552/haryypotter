...
function Character() {
  ...
  useGSAP(
    () => {
      ...
      tl.set(modalOverlayRef.current, { autoAlpha: 0 })
        .set(modalContentRef.current, { autoAlpha: 0, scale: 0.92, y: 16 })
        .to(modalOverlayRef.current, { autoAlpha: 1, duration: 0.2, ease: "power1.out" }, 0)
        .to(modalContentRef.current, { autoAlpha: 1, scale: 1, y: 0, duration: 0.35, ease: "power3.out" }, 0.05)
    ...
    <div className="min-h-screen w-full bg-[rgb(5,5,7)] text-white" ref={containerRef}>
      ...
      {selected && (
        ...
        >
          <div
            ref={modalContentRef}
            className="relative w-full max-w-3xl rounded-2xl border border-yellow-400/20 bg-[rgb(12,12,16)] shadow-2xl overflow-hidden"
          >
            <button
              onClick={() => setSelected(null)}
              className="absolute right-3 top-3 z-10 text-white/70 hover:text-white text-xl"
            >
              ×
            </button>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-0">
              ...
              <div className="p-6 md:p-7">
                <div className="flex items-center justify-between gap-3">
                  ...
                  <span className="text-xs px-2 py-1 rounded-full bg-yellow-400/15 text-yellow-300 border border-yellow-400/20">
                    {selected.relationship}
                  </span>
                </div>

                <blockquote className="mt-4 border-l-2 border-yellow-400/30 pl-3 italic font-serif text-white/90">
                ...
                <div className="mt-4 flex flex-wrap gap-2">
                  {selected.traits?.map((t) => (
                    <span key={t} className="trait-badge text-xs px-2 py-1 font-serif rounded border border-white/10 bg-white/5">
                      {t}
                    </span>
                  ))}
                </div>
                ...
                {selected.extraImages?.length > 0 && (
                  <div className="mt-5 grid grid-cols-2 gap-3">
                    {selected.extraImages.map((img) => (
                      <img key={img} src={img} alt="extra" className="w-full h-24 object-cover rounded-lg border border-white/10" />
                    ))}
                  </div>
                )}
              </div>
...
function Card({ data, onClick }) {
  ...
  const handleClick = () => {
    ...
    tl.set(el, {
      ...
      .to(el, { duration: 0.34, rotateX: 0, z: 0, scale: 1 }, "+=0")
      .add(() => {
        onClick && onClick();
  ...
  return (
    <div style={{ perspective: 1000 }}>
      <div
        onClick={handleClick}
        ref={cardRef}
        className="char-card group transform-gpu rounded-xl overflow-hidden border border-yellow-400/20 bg-white/5 transition-colors cursor-pointer"
      >
        <div
          className="relative aspect-[2/3] bg-black/50"
        ...
        >
          {data.video ? (
            ...
            <img src={data.image} alt={data.name} className="w-full h-full object-cover filter grayscale transition duration-300 group-hover:grayscale-0" />
          )}
        ...
        <div className="p-4">
          ...
          <div className="mt-3 flex flex-wrap gap-2 font-serif">
            {data.traits?.slice(0, 3).map((t) => (
              <span key={t} className="text-xs px-2 py-1 rounded border border-white/10 bg-white/10">
                {t}
              </span>
            ))}
          </div>
